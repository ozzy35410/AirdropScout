      ...
      >
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">{t(task.description)}</p>
          </div>
        </div>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-4">
              ...
              >
                ...
                <span>{showWalletStats ? t('hide_stats') : t('show_wallet_stats')}</span>
              </button>
            </div>
            {trackingAddress && !isValidAddress && (
              <p className="text-xs text-red-600 mt-2">{t('invalid_address_format')}</p>
            )}
            {isValidAddress && (
              <p className="text-xs text-green-600 mt-2">
                {t('tracking')}: {trackingAddress.slice(0, 6)}...{trackingAddress.slice(-4)}
              </p>
            )}
          </div>
              ...
              <div className="p-6">
                {loadingStats && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 mt-4">{t('loading_stats')}</p>
                  </div>
                )}
                ...
                {stats && !loadingStats && (
                  <>
                    ...
                    {stats.txsPreview.length > 0 && (
                      <div>
                        <h3 className="text-lg font-bold text-gray-900 mb-4">{t('recent_transactions')}</h3>
